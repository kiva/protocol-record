...
export class CredentialService {
  ...
  public async getCredentials(institution: string): Promise<Credential[]> {
    ...
    const filter = new Credential();
    filter.institution = institution;
    return await this.credentialRepo.find(filter);
  }
  ...
  public async createCredential(dto: CreateCredentialDto): Promise<Credential> {
    ...
    newCredential.institution = dto.institution;
  ...
  public async revokeCredential(credential_exchange_id: string, dto: RevokeCredentialDto): Promise<Credential> {
    return this.credentialRepo.manager.transaction(async (entityManager: EntityManager) => {
      const cred: Credential = await entityManager.findOne(Credential, {credential_exchange_id});
      if (!cred) {
        throw(new Error('Credential not found in database with credential exchange id: ' + credential_exchange_id));
      }
